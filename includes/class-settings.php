...
CSS;
	...
	public function render_settings_page(): void {
		...
		$mcp_url      = rest_url( 'pressocampus/v1/mcp' );
	...
	public function ajax_test_connection(): void {
		...
		$endpoint = rest_url( 'pressocampus/v1/mcp' );
		...
		$http_label = sprintf( __( 'HTTP %d', 'pressocampus' ), $code );
		$detail     = is_array( $json ) && isset( $json['error']['message'] )
			? $json['error']['message']
			: $http_label;

		/* translators: %s: error detail message */
	...
	public function ajax_revoke_client(): void {
		...
		if ( $client_key ) {
			$wpdb->delete( $tokens_table, array( 'client_id' => $client_key ), array( '%s' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}

		// Delete client record
		$deleted = $wpdb->delete( $clients_table, array( 'id' => $client_id ), array( '%d' ) );
