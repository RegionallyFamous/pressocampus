...
CSS;
    ...
    public function render_settings_page(): void {
        ...
        <?php $this->render_settings_js($mcp_url, $site_name); ?>
        <?php
    }
    ...
    public function render_history_page(): void {
        ...
        $all_actions = ['remember', 'forget', 'update_memory', 'update_soul', 'resources_list', 'resources_read'];
        ...
        <div class="wrap pc-wrap">
            ...
            <?php else : ?>
                ...
                <div class="pc-pagination">
                    <?php if ($page > 1) : ?>
                        <a href="<?php echo esc_url(add_query_arg(['paged' => $page - 1, 'pc_search' => $search, 'pc_agent' => $agent_filter, 'pc_action' => $action_filter], $base_url)); ?>" class="pc-page-btn">← <?php esc_html_e('Prev', 'pressocampus'); ?></a>
                    <?php endif; ?>
                    ...
                    <?php if ($page < $total_pages) : ?>
                        <a href="<?php echo esc_url(add_query_arg(['paged' => $page + 1, 'pc_search' => $search, 'pc_agent' => $agent_filter, 'pc_action' => $action_filter], $base_url)); ?>" class="pc-page-btn"><?php esc_html_e('Next', 'pressocampus'); ?> →</a>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
    ...
    private function render_settings_js(string $mcp_url, string $site_name): void {
    ...
    public function ajax_test_connection(): void {
        ...
        if ($code === 401 || (is_array($json) && array_key_exists('result', $json))) {
            wp_send_json_success(['message' => __('Endpoint is reachable and responding correctly.', 'pressocampus')]);
        }
        ...
        wp_send_json_error(['message' => sprintf(__('Unexpected response: %s', 'pressocampus'), $detail)]);
    }
    ...
    public function ajax_revoke_client(): void {
        ...
        wp_send_json_success(['message' => __('Client revoked.', 'pressocampus')]);
    }
    ...
    public function ajax_export_brain(): void {
        ...
        if (class_exists('ZipArchive')) {
            ...
            if ($zip->open($zip_file, \ZipArchive::OVERWRITE) === true) {
                ...
                readfile($zip_file); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_readfile
                unlink($zip_file);
                exit;
            }
        }
    ...
    public function ajax_save_settings(): void {
        ...
        wp_send_json_success(['message' => __('Settings saved.', 'pressocampus')]);
    }
    ...
    public function ajax_dismiss_notice(): void {
        ...
        if (!str_starts_with($option_name, 'pressocampus_expiry_notice_')) {
            wp_send_json_error(['message' => __('Invalid notice key.', 'pressocampus')]);
        }
    ...
    private function get_settings(): array {
        ...
        $saved = get_option('pressocampus_settings', []);
