...
class AuditLog {
    ...
    public function get_entries(
        ...
        $items_args  = array_merge($values, [$per_page, $offset]);
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    public function get_agent_names(int $user_id = 0): array {
        ...
        return array_filter(array_values($rows ?: []), fn($n) => $n !== '');
    }
    ...
    public function export_csv(int $user_id = 0, int $days = 30): string {
        ...
        $headers = ['ID', 'User ID', 'Agent', 'Action', 'Memory URI', 'Memory Name', 'Context', 'Date'];
